...
def run_timestamps(env, model, timestamps=128, render=False, device="cpu"):
    ...
    for ts in range(timestamps):
        ...
        for agent_idx in np.arange(env.n_agents):
            ...
            state_ = env.get_flattened_state(agent_idx=agent_idx).reshape(*local_dims) #+ ut.add_noise(local_dims)
            state = torch.from_numpy(state_).float().to(device)

            action, act_logits, value = model(state)
        ...
        _, reward, terminated = env.step(actions)
        ...
        if render:
            ...
            for agent_idx in np.arange(env.n_agents):
                buffer.append([states[agent_idx], 
                               actions[agent_idx], 
...
class MyPPO(nn.Module):
    ...
    def __init__(self, in_dim, n_actions, hidden_d=100, share_backbone=False):
        # Super constructor
...
def training_loop(env, model, max_iterations, n_actors, horizon, gamma, epsilon, n_epochs, batch_size, lr,
                  c1, c2, device, use_mp=False, env_name=""):
        ...
        if use_mp:
            ...
            for _ in range(n_actors):
                pool.apply_async(run_timestamps, 
                                args=(env, model, horizon, False, device),
...
def main():
    ...
    env_name = "Multi-Agent-GW"
    env = MultiAgentGridWorld(game_mode='random',
                              size=8,
                              n_agents=3)
    actor_dims, _ = env.get_env_dims()
    ...
    training_loop(env, model, args["max_iterations"], args["n_actors"], args["horizon"], args["gamma"], args["epsilon"],
                  args["n_epochs"], args["batch_size"], args["lr"], args["c1"], args["c2"], device, 
                  args["use_mp"], env_name=env_name)
